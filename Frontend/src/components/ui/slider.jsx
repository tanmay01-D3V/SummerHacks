...
const Slider = React.forwardRef(({ className, showTooltip = false, tooltipContent, ...props }, ref) => {
  ...
  const [internalValue, setInternalValue] = React.useState(
    props.defaultValue ?? props.value ?? [-1],
  )
  ...
  return (
    ...
    >
      <SliderPrimitive.Track
        ...
        className="relative grow overflow-hidden rounded-full bg-emerald-200 data-[orientation=horizontal]:h-2 data-[orientation=vertical]:h-full data-[orientation=horizontal]:w-full data-[orientation=vertical]:w-2"
      >
        <SliderPrimitive.Range
          data-slot="slider-range"
          className="absolute bg-emerald-600 data-[orientation=horizontal]:h-full data-[orientation=vertical]:w-full"
        />
      </SliderPrimitive.Track>
